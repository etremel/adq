...
template<typename Base, int PrecisionBits>
struct hash<adq::util::FixedPoint<Base, PrecisionBits>> {
        size_t operator()(const adq::util::FixedPoint<Base, PrecisionBits>& input) const {
            return std::hash<Base>()(static_cast<Base>(input));
        }
};

template<typename Base, int PrecisionBits, typename Allocator>
struct hash<vector<adq::util::FixedPoint<Base, PrecisionBits>, Allocator>> {
        size_t operator()(const vector<adq::util::FixedPoint<Base, PrecisionBits>, Allocator>& input) const {
