...
struct ByteRepresentable {
     ...
     */
    virtual std::size_t to_bytes(unsigned char* v) const = 0;
    ...
    virtual void post_object(
            const std::function<void(unsigned char const* const, std::size_t)>&) const = 0;
    ...
    // virtual static std::unique_ptr<T> from_bytes(DeserializationManager *p, const unsigned char *v) const  = 0;
     ...
     */
    // needs to exist, but can't declare virtual statics
    // virtual static context_ptr<T> from_bytes_noalloc(DeserializationManager *p, const unsigned char *v) const  = 0;
};
...
std::enable_if_t<std::is_pod<BR>::value> post_object(const F& f, const BR& br,
                                                     Args&&... args) {
    f(std::forward<Args>(args)..., (unsigned char*)&br, sizeof(BR));
}

void post_object(const std::function<void(unsigned char const* const, std::size_t)>& f,
 ...
 */
std::size_t to_bytes(const ByteRepresentable& b, unsigned char* v);

/**
 * extracts the C string (char*) equivalent to this
 * std::string and stores it in v
 */
std::size_t to_bytes(const std::string& b, unsigned char* v);
...
std::enable_if_t<std::is_base_of<ByteRepresentable CMA T>::value,
                 std::unique_ptr<T>>
from_bytes(DeserializationManager* ctx, unsigned char const* v) {
...
from_bytes(DeserializationManager*, unsigned char const* v);
...
std::enable_if_t<std::is_base_of<ByteRepresentable CMA std::decay_t<T>>::value,
                 context_ptr<T>>
from_bytes_noalloc(
        DeserializationManager* ctx, unsigned char* v,
...
std::enable_if_t<std::is_base_of<ByteRepresentable CMA std::decay_t<T>>::value,
                 context_ptr<T>>
from_bytes_noalloc(
        DeserializationManager* ctx, unsigned char const* const v,
...
from_bytes_noalloc(DeserializationManager*, unsigned char* v);

template <typename T>
std::enable_if_t<std::is_pod<T>::value, context_ptr<const std::decay_t<T>>>
from_bytes_noalloc(DeserializationManager*, unsigned char const* const v,
...
auto deserialize_and_run(DeserializationManager* dsm, unsigned char* v, const F& fun);
...
struct marshalled : public ByteRepresentable {
    ...
    std::size_t to_bytes(unsigned char* v) const {
    ...
    from_bytes_noalloc(DeserializationManager const* const, unsigned char* v);
};
...
post_to_buffer(std::size_t& index, unsigned char* dest_buf);
...
std::size_t to_bytes(const std::vector<bool>& vec, unsigned char* buffer);

template <typename T>
std::size_t to_bytes(const std::vector<T>& vec, unsigned char* buffer);

template <typename T>
std::size_t to_bytes(const std::list<T>& list, unsigned char* buffer);

template <typename T, typename V>
std::size_t to_bytes(const std::pair<T, V>& pair, unsigned char* buffer);

template <typename... T>
std::size_t to_bytes(const std::tuple<T...>& tuple, unsigned char* buffer);

template <typename T>
std::size_t to_bytes(const std::set<T>& s, unsigned char* _v);

template <typename K, typename V>
std::size_t to_bytes(const std::map<K, V>& m, unsigned char* buffer);

template <typename K, typename V>
std::size_t to_bytes(const std::unordered_map<K, V>& m, unsigned char* buffer);
...
from_bytes_noalloc(DeserializationManager*, unsigned char const* v,
                   context_ptr<T> = context_ptr<T>{});
...
from_bytes_noalloc(DeserializationManager* ctx, unsigned char const* v,
                   context_ptr<T> = context_ptr<T>{});
...
template <typename T>
std::unique_ptr<type_check<is_tuple, T>> from_bytes(DeserializationManager* ctx, unsigned char const* v);
...
from_bytes(DeserializationManager* ctx, unsigned char const* buffer);
...
void post_object_helper(
        const std::function<void(unsigned char const* const, std::size_t)>& f,
        const T&... t) {
    (post_object(f, t), ...);
}
...
void post_object(const std::function<void(unsigned char const* const, std::size_t)>& f,
    ...
    f((unsigned char*)&size, sizeof(size));
    if(std::is_pod<T>::value) {
        std::size_t size = vec.size() * bytes_size(vec.back());
        f((unsigned char*)vec.data(), size);
    } else {
 ...
 */
template <typename T, restrict(std::is_pod<T>::value)>
std::size_t to_bytes(const T& t, unsigned char* v) {
...
std::size_t to_bytes(const std::vector<bool>& vec, unsigned char* v);

template <typename T>
std::size_t to_bytes(const std::vector<T>& vec, unsigned char* v) {
...
from_string(DeserializationManager*, unsigned char const* v, std::size_t length) {
...
std::unique_ptr<type_check<is_set, T>> from_bytes(DeserializationManager* ctx,
                                                  const unsigned char* _v) {
    int size = ((int*)_v)[0];
    const unsigned char* v = _v + sizeof(int);
...
std::unique_ptr<type_check<is_list, L>> from_bytes(DeserializationManager* ctx,
    ...
    const unsigned char* buf_ptr = buffer + sizeof(int);
...
auto from_bytes_helper(DeserializationManager* ctx, unsigned char const* v) {
...
std::size_t to_bytes_v(unsigned char*);

template <typename T, typename... Rest>
std::size_t to_bytes_v(unsigned char* buf, const T& first, const Rest&... rest) {
    auto size = to_bytes(first, buf);
    return size + to_bytes_v(buf + size, rest...);
}

std::size_t from_bytes_v(DeserializationManager*, unsigned char const* const);

template <typename T, typename... Rest>
std::size_t from_bytes_v(DeserializationManager* dsm, unsigned char const* const buf,
...
std::size_t from_bytes_noalloc_v(DeserializationManager*, unsigned char const* const);

template <typename T, typename... Rest>
std::size_t from_bytes_noalloc_v_nc(DeserializationManager* dsm, unsigned char* buf,
                                    context_ptr<T>& first,
...
template <typename T, typename... Rest>
std::size_t from_bytes_noalloc_v(DeserializationManager* dsm, unsigned char* buf,
                                 context_ptr<T>& first,
                                 context_ptr<Rest>&... rest) {
...
auto deserialize_and_run(DeserializationManager* dsm, unsigned char const* const v,
    ...
    std::tuple<DeserializationManager*, const unsigned char*,
    ...
    // help the compiler
    using from_bytes_type = std::size_t (*)(DeserializationManager*, const unsigned char*,
                                            context_ptr<const std::decay_t<Args>>&...);
    from_bytes_type from_bytes_noalloc_concrete =
            [](DeserializationManager* d, const unsigned char* c,
