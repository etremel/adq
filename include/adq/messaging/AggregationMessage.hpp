...
struct hash<adq::messaging::AggregationMessageValue> {
    size_t operator()(const adq::messaging::AggregationMessageValue& input) const {
        ...
        for (adq::FixedPoint_t::base_type i : input) {
...
template<>
struct hash<adq::messaging::AggregationMessage> {
        size_t operator()(const adq::messaging::AggregationMessage& input) const {
            using adq::util::hash_combine;

            size_t result = 1;
            hash_combine(result, input.num_contributors);
            hash_combine(result, input.query_num);
            hash_combine(result, *std::static_pointer_cast<adq::messaging::AggregationMessageValue>(input.body));
            return result;
        }
};
